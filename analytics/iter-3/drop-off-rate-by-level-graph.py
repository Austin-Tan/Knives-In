...
OUT_FILE = 'csv/drop-off-rate-by-level-B.csv'
...
for uid, session_map in data_map.items():
   # qid to sessionid map
   qid_to_dqid_map = {}
   for sessionid, data in session_map.items():
      for row in data:
         qid = row[index['qid']]
         dqid = row[index['dqid']]
         q_s_id = row[index['q_s_id']]
         if q_s_id == '1': # if this is the start of the level
            qid_to_dqid_map[int(qid)] = dqid
         
         if dqid == 'NULL':
            row[index['qid']] = '{}'.format(int(qid) - 1)
            if int(qid) - 1 not in qid_to_dqid_map:
               count += 1
            else:
               row[index['dqid']] = qid_to_dqid_map[int(qid) - 1]
...
for uid, sessions in data_map.items():
   ...
   for k, v in level_instance_map.items():
      if 'level' in v:
         ...
         # if len(v) == 4 and v['level'] == 11 and v['stage'] == 1:
         #    last_level = True

   # if not last_level:
   uid_to_best_level[uid] = float('{}.{}'.format(max_level, max_stage))
   # else:
   #    uid_to_best_level[uid] = 11 # completed
